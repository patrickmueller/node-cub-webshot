...
var _takeScreenshot = function(status, type) {
	if (status === 'fail') {
		page.close();
		phantom.exit(1);
		return;
	...
	window.setTimeout(function () {
		...
		// Inject counter number into path
		var outputPath = path.split('/')
		var fileName = outputPath[outputPath.length - 1].split('.');
		fileName[0] = fileName[0] + '-' + counter;
		outputPath[outputPath.length - 1] = fileName.join('.');
		outputPath = outputPath.join('/');

		// Render, clean up, and exit
		if (!streaming) {
			page.render(outputPath, {quality: options.quality});
			console.log('Screenshot ' + counter);
			counter++;
		} else {
...
if (options.onCallback && options.takeShotOnCallback) {
	takeScreenshot = function(data) {
		...
		if (data == 'takeShot') {
			_takeScreenshot(null, 'continue');
		} else if (data == 'lastShot') {
			_takeScreenshot(null, 'last');
		}
	};
